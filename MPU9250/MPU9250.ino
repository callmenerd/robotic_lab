...
void loop() {
  current_time = millis();
  if(current_time-prev_time>input_period){
    ...
    filter.updateIMU(gyr.x, gyr.y, gyr.z, gValue.x, gValue.y, gValue.z);
    ...
    Serial.print("\tQZ: ");
    Serial.print(q3);
    float yaw = atan2(2 * (q1 * q2 + q0 * q3), q0 * q0 + q1 * q1 - q2 * q2 - q3 * q3);
    float pitch = asin(-2 * (q1 * q3 - q0 * q2));
    float roll = atan2(2 * (q0 * q1 + q2 * q3), q0 * q0 - q1 * q1 - q2 * q2 + q3 * q3);
    Serial.print(" | Sudut (x,y,z): ");
