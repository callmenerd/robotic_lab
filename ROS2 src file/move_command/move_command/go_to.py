...
class Movement_Plan(Node):
    def __init__(self):
        ...
        #Path planning
        self.path_from_start = [[[5, 620, 0], [415, 610, 0], [410, 950, 0], [50, 950, 0]],
                                 [[150, 85, 0], [415, 85, 0], [420, 430, 0], [50, 430, 0]],
                                 [[0, 620, 0], [-442, 610, 0], [-437, 950, 0], [-100, 950, 0]],
                                 [[-150, 85, 0], [-442, 85, 0], [-435, 430, 0], [-100, 430, 0]]]
        self.idx = 0
        self.start_index = 0
        ...
        self.US_PID = PID(0.325, 0.00, 0.005, setpoint=0, auto_mode=True, output_limits=(-0.69, 0.69))
            ...
            if self.enable_move:
                ...
                if self.start_index == 0:
                    if self.idx == 0 and abs(Y) > 5:
                        ...
                        new_vel.linear.x = Vx*0.7
                        new_vel.linear.y = -self.US_PID((self.us_distance[1]-37)/37)
                    elif self.idx == 2 and abs(Y) > 5:
                        new_vel.linear.x = 0.110
                        new_vel.linear.y = Vy
                    elif self.idx == 3 and abs(X) > 20:
                        new_vel.linear.x = Vx
                        new_vel.linear.y = Vy
                    else :
                    ...
                    if self.idx == 0 and abs(X) > 5:
                        ...
                        if self.idx < 3:
                            ...
                            self.get_logger().info("Goal Reached")
                            self.activate.data = True
                elif self.start_index == 2:
                    if self.idx == 0 and abs(Y) > 5:
                        new_vel.linear.y = Vy*0.85
                        new_vel.linear.x = -self.US_PID((self.us_distance[2]-27)/27)
                    elif self.idx == 1 and abs(X) > 5:
                        new_vel.linear.x = Vx*0.7
                        new_vel.linear.y = -self.US_PID((self.us_distance[1]-37)/37)
                    elif self.idx == 2 and abs(Y) > 5:
