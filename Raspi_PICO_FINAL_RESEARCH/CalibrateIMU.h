...
//=============================================================================================
#ifndef __CalibrateIMU_h__
#define __CalibrateIMU_h__
#include <Arduino.h>
#include <math.h>
#include <stdint.h>
...
class CalibrateIMU{
  ...
  public:
    ...
    void get_acc_biass(){
      this->acc_bias[0] = accel_bias[0];
      this->acc_bias[1] = accel_bias[1];
      this->acc_bias[2] = accel_bias[2];
    }
    void begin(void){
    ...
    void getAllUncalibratedData(){
        ...
        this->raw_accel[0] = accel_event.acceleration.x-this->accel_bias[0];
        this->raw_accel[1] = accel_event.acceleration.y-this->accel_bias[1];
        this->raw_accel[2] = accel_event.acceleration.z-this->accel_bias[2]+9.8067;
    }
    void getAllDataCalibrated(){
      if (this->init){
        int n = 32;
        this->accel_bias[0] = 0; this->accel_bias[1] = 0; this->accel_bias[2] = 0;
        while (n >= 0){
    ...
    void UpdateGyroAccCalibration(unsigned long duration){
      ...
      while ((millis() - start_time) < duration){
          ...
          accels_bias[0] += accel_event.acceleration.x;
          accels_bias[1] += accel_event.acceleration.y;
          accels_bias[2] += accel_event.acceleration.z;
      ...
      this->accel_bias[0] = accels_bias[0] / sample_count;
      this->accel_bias[1] = accels_bias[1] / sample_count;
      this->accel_bias[2] = (accels_bias[2] / sample_count);
    }
    ...
    void calibrate_accelerometer_readings(){
      ...
      float filtered_cax = this->alpha_accel * this->raw_accel[0] + (1 - this->alpha_accel) * this->accel[0];
      float filtered_cay = this->alpha_accel * this->raw_accel[1] + (1 - this->alpha_accel) * this->accel[1];
      float filtered_caz = this->alpha_accel * this->raw_accel[2] + (1 - this->alpha_accel) * this->accel[2];
