...
class SensorFusionNode(Node):
    def __init__(self):
        ...
        self.odom_filtered_pub = self.create_publisher(Float32MultiArray, 'sensor/odom_filtered', qos_profile)
    ...
    def update_filter(self):
        ...
        final_pose.data.append(self.x[0])
        final_pose.data.append(self.x[1])
        final_pose.data.append(self.x[2])
        final_pose.data.append(self.z_odom[3])
        final_pose.data.append(self.z_odom[4])
        final_pose.data.append(self.z_odom[5])
        # final_pose.twist.twist.linear.x = self.x[3]
        # final_pose.twist.twist.linear.y = self.x[4]
        # final_pose.twist.twist.angular.z = self.x[5]

        self.odom_filtered_pub.publish(final_pose)

        self.new_odom_data = False
        self.new_imu_data = False
        #self.get_logger().info(f"X: {self.final_coord[0]:.2f} | Y: {self.final_coord[1]:.2f} | Theta: {self.x[2]:.2f}")
